...
- El cliente realizó un escaneo automatizado de vulnerabilidades estructurado en etapas: primero reconocimiento de servicio, luego recolección de metadatos, seguido de enumeración forzada (fuzzing) y finalmente exfiltración de configuración de control de versiones.
- La IP `127.0.0.1` podría tratarse de un proxy inverso o de un atacante ejecutando localmente.

### C. Mapeo a Categorías STRIDE
- `GET /` (NSE): **Information Disclosure** (reconocimiento de tecnologías).
- `HEAD /public-inventory.txt`: **Information Disclosure** (verificación de existencia de recursos sensibles).
- Secuencia de 404s: **Information Disclosure / Tampering** (intento de descubrimiento de endpoints no publicados).
- `GET /.git/config` (200 OK): **Information Disclosure Crítico** (exposición de metadatos del repositorio).

### D. Hipótesis Defensivas Propuestas
1. *Hipótesis 1:* La exposición de `/.git/config` permite a un actor externo reconstruir el historial de commits y potenciales credenciales hardcodeadas.
...
- Faltan logs de integridad de archivos para saber si el repositorio Git fue clonado en su totalidad mediante herramientas como `git-dumper`.
